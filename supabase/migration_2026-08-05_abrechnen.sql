...
-- "Jetzt Abrechnen"-Workflow auf der Lohnübersicht: markiert die Saison für
-- eine Person als abgerechnet (pro Saison-Jahr) und setzt employees.aktiv
-- auf false, atomar über eine kontrollierte Funktion (damit lohnabrechnung
-- das auslösen kann, ohne dass die employees-Tabelle generell für diese
-- Rolle geöffnet werden muss).
-- ============================================================================

alter table season_bonuses
  add column abgerechnet_am timestamptz,
  add column abgerechnet_von uuid references profiles (id);
...
-- season_summary neu anlegen mit aktiv + abgerechnet_am
...
-- "Zuletzt abgerechnet am" für die Personal-Seite (nur das Datum, keine
-- Beträge - damit auch hr das sehen kann, ohne season_bonuses lesen zu
-- dürfen).
