...
create table stundenkonto_bewegungen (
  ...
  -- gab es KEINEN Weg, eine "In Auszahlung umwandeln"-Aktion rückgängig zu
  -- machen. betrag hält den bei der Umwandlung tatsächlich berechneten
  -- Auszahlungsbetrag fest (Stunden × zu dem Zeitpunkt gültiger
  -- Stundenlohn) - nötig, damit eine spätere Stornierung exakt denselben
  -- Betrag aus season_bonuses.stundenkonto_auszahlung_betrag wieder
  -- herausrechnen kann, auch falls sich der Stundenlohn der Person
  -- inzwischen geändert hat. Nur bei art = 'Auszahlung' befüllt. storniert
  -- verhindert eine doppelte Stornierung derselben Zeile - siehe
  -- stundenkonto_auszahlung_stornieren weiter unten.
  betrag numeric(10, 2),
  storniert boolean not null default false
);
...
-- Macht eine "In Auszahlung umwandeln"-Aktion rückgängig (Nutzer-Vorgabe
-- 2026-08-28: "Ich habe ein Stundenkonto fälschlicherweise in Auszahlung
-- umgewandelt, jetzt stehen sie als Zulage im Lohn und ich bekomme sie
-- nicht mehr raus"). Bucht die Stunden NICHT durch Löschen der
-- ursprünglichen Zeile zurück (Audit-Trail bliebe sonst unvollständig),
-- sondern durch eine zusätzliche, ausgleichende 'Korrektur'-Bewegung
-- (positiv, ohne Saldo-Prüfung) - die ursprüngliche 'Auszahlung'-Zeile
-- bleibt sichtbar stehen, wird nur als storniert markiert. Zieht denselben
-- Betrag (aus der gespeicherten betrag-Spalte, nicht neu berechnet) wieder
-- von season_bonuses.stundenkonto_auszahlung_betrag ab. Ist die Person
-- bereits abgerechnet, exakt dasselbe Schnappschuss-/Kassenbewegungs-
-- Muster wie bei der Umwandlung selbst bzw. abrechnung_korrigieren.
...
-- ---------------------------------------------------------------------------
-- 6a0. Firmen-Bankdaten (Nutzer-Vorgabe 2026-08-25): Auftraggeber-Konto für
...
-- ---------------------------------------------------------------------------
-- 10a. Mehrere Kassenbücher (Nutzer-Vorgabe). Phase 1: Bücher + Journal je
--      Buch + Umbuchungen zwischen zwei Büchern. "Mömmel Lohnkasse" bleibt
--      unangetastet - ihr Saldo kommt weiter aus kassenbestand_bis(),
--      zusätzlich fließen Umbuchungen von/zu ihr über kassenbuch_buchung ein.
--      Phase 2 (separat): Kassenprüfung je Buch + Eröffnungssaldo-Sperre nach
--      erster freigegebener Prüfung.
-- ---------------------------------------------------------------------------
create table kassenbuch (
  id bigint generated always as identity primary key,
  bezeichnung text not null unique,
  kuerzel text not null unique,           -- Präfix der Belegnummern (MLK-2610-001)
  typ text not null check (typ in ('lohnkasse', 'allgemein')),
  eroeffnungssaldo numeric(12, 2) not null default 0,  -- admin-editierbar (Phase 2 sperrt es)
  aktiv boolean not null default true,
  reihenfolge int not null default 0,
  erstellt_am timestamptz not null default now(),
  updated_at timestamptz not null default now()
...
-- Kassenprüfung je Buch (Phase 2, Migration 2026-10-04): cash_checks bekommt
-- kassenbuch_id. Als nachgelagertes alter, weil cash_checks weiter oben (10.)
-- definiert wird, kassenbuch aber erst hier.
alter table cash_checks
  add column if not exists kassenbuch_id bigint references kassenbuch (id);
update cash_checks set kassenbuch_id = (select id from kassenbuch where typ = 'lohnkasse')
...
-- Saldo je Buch. allgemein: eroeffnungssaldo + Σ eingang − Σ ausgang;
-- lohnkasse: zusätzlich die bestehende 4-Quellen-Formel kassenbestand_bis().
...
returns numeric language sql stable as $$
  select
    ...
    + coalesce((
        ...
        where b.kassenbuch_id = kb.id and not b.storniert and b.datum < p_bis
      ), 0)
...
-- Storno einer Buchung (Pflichtgrund). Bei einer Umbuchung wird immer auch die
-- Gegenbuchung mit storniert.
...
-- Gilt nur für die Mömmel Lohnkasse (typ = 'lohnkasse'): die 5 Aufrufstellen
-- im Lohn-/Vorschuss-Code und die RLS-Policies auf advances/cash_deposits
-- meinen weiterhin genau diese Kasse. Freigegebene Prüfungen der übrigen
-- Bücher sperren die Lohn-/Vorschuss-Belege nicht mit
-- (siehe ist_kassenbuch_pruefung_gesperrt für die allgemeinen Bücher).
...
-- Für die "allgemeinen" Kassenbücher: ist das Kassenbuch-Datum durch eine
-- freigegebene Prüfung genau dieses Buchs gesperrt? Genutzt in
-- kassenbuch_buchen / kassenbuch_umbuchen / kassenbuch_buchung_stornieren.
...
-- ---------------------------------------------------------------------------
-- 12a2. Views: Saison-Historie einer Person ("wann war sie schon da") -
--       vereint die aus der Papier-Personalkartei importierten Jahre
--       (employee_saison_praesenz) mit den von der App selbst erfassten
--       Saisons (work_entries). Genutzt in Personalplanung, Personalstamm und
--       der druckbaren Personalstammkarte.
-- ---------------------------------------------------------------------------
...
--       Kalendermonat der Beschäftigung. Ein Kalendermonat zählt als "voll"
--       (Fix 2026-09-24), wenn in seinen ersten 3 UND letzten 3 Tagen je
--       mindestens ein Eintrag (Stunden ODER Markierung) liegt - die
--       3-Tage-Toleranz fängt Wochenenden/Feiertage an den Monatsrändern ab.
--       Damit zählen leere Monate, angebrochene Rand-Monate UND mitten im
--       Monat begonnene/beendete Monate (z.B. Austritt am 25.) NICHT mit.
--       (Historie: 2026-09-23 zählte noch die Spanne erster..letzter Eintrag,
--       nur rand-getrimmt.) "U"-markierte Tage sind die
...
join lateral (
  ...
  -- letzten 3 Tagen je mindestens ein Eintrag liegt (Toleranz für
  -- Wochenenden/Feiertage an den Monatsrändern). Fix 2026-09-24.
...
-- ===========================================================================
-- 15. Modul "Unterkunft" (Zimmerverwaltung)
--
--     Deckungsgleich mit supabase/migration_2026-08-29_unterkunft.sql - dort
--     stehen die ausführlichen Begründungen. Kurzfassung:
--       - Zimmerplanung Gebäude > Zimmer > Bett > Belegung (bettgenau, mit
--         EXCLUDE-Constraint gegen Doppelbelegung, braucht btree_gist)
--       - Übergabe/Abnahme/Zwischenkontrolle je Zimmer mit versionierter
--         Checkliste, Fotodokumentation (Bucket "unterkunft-fotos") und
--         Mängelerfassung
--       - abgeschlossene Vorgänge sind append-only (Schutz-Trigger), Storno
--         nur durch admin (ADR-011)
--       - Pflege admin + hr (RLS), lesen alle Angemeldeten
-- ===========================================================================
...
-- mehrere Zimmer + gemeinsam Bad/Küche. Die "Etage" ist nur noch ein
-- Text-Label darauf (Umschalter in der UI). Ersetzt die kurzlebige Tabelle
-- unterkunft_etage aus Migration 2026-08-30.
...
create table unterkunft_zimmer (
  ...
  etage text,
  wohneinheit_id bigint references unterkunft_wohneinheit (id) on delete set null,
...
create table unterkunft_vorgang (
  ...
  gesamtzustand text check (
    gesamtzustand is null or gesamtzustand in ('gut', 'gebrauchsspuren', 'maengel')
  ),
  notiz text,
...
-- Herkunfts-Zimmerplanung (Migration 2026-09-18): der Planer legt je Herkunft
-- eine Planzahl (soll_anzahl) fest und stapelt Wohneinheiten als Stammplätze
-- (unterkunft_herkunft_kontingent, geordnet nach reihenfolge). Beim Aktivieren
-- eines Kandidaten füllt die Personalplanung diese Häuser der Reihe nach mit
-- echten Namen (geplante unterkunft_zuordnung).
create table unterkunft_herkunft_plan (
  saison_jahr int not null,
...
create table unterkunft_herkunft_kontingent (
  id bigint generated always as identity primary key,
  saison_jahr int not null,
  herkunft text not null references herkuenfte (wert) on delete restrict,
  wohneinheit_id bigint not null references unterkunft_wohneinheit (id) on delete cascade,
  reihenfolge int not null default 0,
  notiz text,
  erfasst_von uuid references profiles (id) default auth.uid(),
  erfasst_am timestamptz not null default now(),
...
create index idx_unterkunft_herkunft_kontingent_saison
  on unterkunft_herkunft_kontingent (saison_jahr, herkunft);
create trigger trg_unterkunft_herkunft_plan_updated_at before update on unterkunft_herkunft_plan
  for each row execute function set_updated_at();
create trigger trg_unterkunft_herkunft_kontingent_updated_at before update on unterkunft_herkunft_kontingent
  for each row execute function set_updated_at();
...
create policy "unterkunft_herkunft_plan_write" on unterkunft_herkunft_plan for all
  using (current_role_name() in ('admin', 'hr'))
  with check (current_role_name() in ('admin', 'hr'));
create policy "unterkunft_herkunft_kontingent_select" on unterkunft_herkunft_kontingent for select
  using (current_role_name() in ('admin', 'hr', 'hausmeister', 'management'));
create policy "unterkunft_herkunft_kontingent_write" on unterkunft_herkunft_kontingent for all
  using (current_role_name() in ('admin', 'hr'))
  with check (current_role_name() in ('admin', 'hr'));
...
begin
  if v_vorgang is not null
     ...
     and exists (
       select 1 from unterkunft_vorgang v where v.id = v_vorgang and v.abgeschlossen
     )
  then
    raise exception
...
-- Lebender Abgleich Planzahl ↔ echte Personen je Herkunft (Migration
-- 2026-09-18). erfasst = angelegte Kandidaten der Saison; verplant = davon
-- mit geplanter Zuordnung; eingezogen = davon mit laufender Belegung.
create view unterkunft_herkunft_abgleich as
select
  p.saison_jahr,
  p.herkunft,
  p.soll_anzahl,
...
select
  b.id as belegung_id,
  b.employee_id,
  e.personal_nr, e.name, e.vorname, e.aktiv as employee_aktiv,
...
-- Zuordnung nur abschliessen/stornieren (im Rahmen der Uebergabe).
...
-- Stufe 2 (Migration 2026-09-26): Geofences + Hofzeiten + Alarm ausserhalb
-- der Arbeitszeit. Geofences kommen aus Traccar (Poller spiegelt sie),
-- fahrzeug_ereignis füllt der Poller aus den Traccar-Events.
create table fahrzeug_geofence (
...
create table fahrzeug_arbeitszeit (
...
insert into fahrzeug_arbeitszeit (wochentag, aktiv, von, bis) values
  (0, true, '06:00', '18:00'), (1, true, '06:00', '18:00'),
  (2, true, '06:00', '18:00'), (3, true, '06:00', '18:00'),
  (4, true, '06:00', '18:00'), (5, true, '06:00', '14:00'),
  (6, false, '06:00', '18:00')
on conflict (wochentag) do nothing;

create table fahrzeug_ereignis (
...
create index idx_fahrzeug_ereignis_fahrzeug
  on fahrzeug_ereignis (fahrzeug_id, zeitpunkt desc);
create index idx_fahrzeug_ereignis_alarm_offen
...
-- ---------------------------------------------------------------------------
-- Firmenhandy-Nummernpool (Migration 2026-09-17, Nutzer-Vorgabe): ~200
-- Rufnummern, deren Nutzer sich in der Saison mehrmals ändern kann. Wird per
-- CardDAV automatisch in ein selbst gehostetes Nextcloud-Adressbuch
-- gespiegelt (siehe app/api/firmenhandy-sync), damit die Smartphones der
-- Mitarbeiter (per DAVx5) immer den aktuellen Namen zur Nummer zeigen -
-- bewusst NICHT über Google. Schlank gehalten ("Minimum: Nummer + aktueller
-- Inhaber", keine Historie, keine Geräte-/SIM-Details).
-- ---------------------------------------------------------------------------
create table firmenhandy (
  id bigint generated always as identity primary key,
...
create index idx_firmenhandy_employee on firmenhandy (employee_id);
