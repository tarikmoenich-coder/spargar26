...
const VERGLEICHS_FELDER: { key: keyof SvFragebogen; label: string }[] = [
...
function bestandenVorschau(e: Entwurf): boolean {
  return (
...
export default function SozialversicherungPage() {
  ...
  async function load() {
    ...
    (frag as SvFragebogenAuswertung[] | null ?? []).forEach((f) => {
      map[f.employee_id] = f;
    });
    setFragebogen(map);
    const mapVorjahr: Record<string, SvFragebogenAuswertung> = {};
    (fragVorjahr as SvFragebogenAuswertung[] | null ?? []).forEach((f) => {
  ...
  function bearbeiten(emp: Employee) {
    setFehler(null);
    setEditingId(emp.id);
    const bestehend = fragebogen[emp.id];
    if (bestehend) {
      setEntwurf({ ...bestehend });
    } else {
      setEntwurf(LEERER_ENTWURF);
    }
    ladeVorbeschaeftigung(bestehend?.id ?? null);
  }

  async function ladeVorbeschaeftigung(fragebogenId: number | null) {
    if (!fragebogenId) {
    ...
    const { data } = await supabase
      ...
      .order("von");
    setVorbeschaeftigung((data as SvFragebogenVorbeschaeftigung[]) ?? []);
  }
  ...
  function vorbeschaeftigungZeileAendern(
    idx: number,
    feld: keyof SvFragebogenVorbeschaeftigung,
    wert: string
  ) {
    setVorbeschaeftigung((prev) =>
      prev.map((z, i) =>
        i === idx
          ? {
              ...z,
              [feld]: feld === "wochenstunden" ? (wert ? Number(wert) : null) : wert,
            }
          : z
      )
    );
  ...
  async function speichern() {
    ...
    const fragebogenId = gespeichert.id as number;
          ...
          <tbody>
            {employees.map((emp) => {
              ...
              const aenderungen = fVorjahr
                ? VERGLEICHS_FELDER.filter(
                    (v) =>
                      !!f?.[v.key] !== !!fVorjahr[v.key]
                  )
                : [];
              return (
                <Fragment key={emp.id}>
                        ...
                        <div className="flex flex-col gap-3 rounded border border-emerald-200 bg-emerald-50 p-4">
                          <div className="flex items-center justify-between">
                            <h3 className="text-sm font-semibold text-emerald-800">
                              SV-Fragebogen {jahr} - {emp.name}, {emp.vorname}
                            </h3>
                            <span
                          ...
                          {fVorjahr && (
                            <p className="text-xs text-neutral-600">
                              Vorjahr ({jahr - 1}):{" "}
                              {VERGLEICHS_FELDER.map(
                                (v) => `${v.label}: ${jaNein(fVorjahr[v.key] as boolean | null)}`
                              ).join(" · ")}
                            </p>
                          )}
                          ...
                          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                            {checkboxZeile(
                              ...
                              <>
                                <input
                                  ...
                                  onChange={(e) =>
                                    feld("beschaeftigt_firma", e.target.value)
                                  }
                                  disabled={!canEdit}
                                />
                                <input
                                  ...
                                  onChange={(e) =>
                                    feld("beschaeftigt_taetigkeit", e.target.value)
                                  }
                                  disabled={!canEdit}
                                />
                                <label className="flex items-center gap-1 text-xs">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      feld("bezahlter_urlaub", e.target.checked)
                                    }
                                    disabled={!canEdit}
                                  />
                                  bezahlter Urlaub
                                ...
                                {entwurf.bezahlter_urlaub && (
                                  <>
                                    <input
                                      ...
                                      onChange={(e) =>
                                        feld("bezahlter_urlaub_von", e.target.value)
                                      }
                                      disabled={!canEdit}
                                    />
                                    <input
                                      ...
                                      onChange={(e) =>
                                        feld("bezahlter_urlaub_bis", e.target.value)
                                      }
                                      disabled={!canEdit}
                                    />
                                  </>
                                ...
                                <label className="flex items-center gap-1 text-xs">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      feld("unbezahlter_urlaub", e.target.checked)
                                    }
                                    disabled={!canEdit}
                                  />
                                  unbezahlter Urlaub
                                ...
                                <label className="flex items-center gap-1 text-xs">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      feld("freistellung", e.target.checked)
                                    }
                                    disabled={!canEdit}
                                  />
                                ...
                                {entwurf.freistellung && (
                                  <>
                                    <input
                                      ...
                                      onChange={(e) =>
                                        feld("freistellung_von", e.target.value)
                                      }
                                      disabled={!canEdit}
                                    />
                                    <input
                                      ...
                                      onChange={(e) =>
                                        feld("freistellung_bis", e.target.value)
                                      }
                                      disabled={!canEdit}
                                    />
                                    <input
                                      ...
                                      onChange={(e) =>
                                        feld("freistellung_grund", e.target.value)
                                      }
                                      disabled={!canEdit}
                                    />
                                  </>
                            ...
                            {checkboxZeile(
                              ...
                              <>
                                <input
                                  ...
                                  onChange={(e) =>
                                    feld("selbststaendig_seit", e.target.value)
                                  }
                                  disabled={!canEdit}
                                />
                                <input
                            ...
                            {checkboxZeile(
                              ...
                              <>
                                <input
                                  ...
                                  onChange={(e) =>
                                    feld("arbeitslos_seit", e.target.value)
                                  }
                                  disabled={!canEdit}
                                />
                                <input
                                  ...
                                  onChange={(e) =>
                                    feld("arbeitsamt_name", e.target.value)
                                  }
                                  disabled={!canEdit}
                                />
                                <input
                            ...
                            {checkboxZeile(
                              ...
                              <>
                                <input
                                  ...
                                  onChange={(e) =>
                                    feld("schule_seit", e.target.value)
                                  }
                                  disabled={!canEdit}
                                />
                                <input
                                  ...
                                  onChange={(e) =>
                                    feld("schule_name", e.target.value)
                                  }
                                  disabled={!canEdit}
                                />
                                <input
                                  ...
                                  onChange={(e) =>
                                    feld("schule_ende", e.target.value)
                                  }
                                  disabled={!canEdit}
                                />
                                <label className="flex items-center gap-1 text-xs">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      feld(
                                        "schulferien_waehrend_beschaeftigung",
                                        e.target.checked
                                      )
                                    }
                                    disabled={!canEdit}
                                ...
                                {entwurf.schulferien_waehrend_beschaeftigung && (
                                  <>
                                    <input
                                      ...
                                      onChange={(e) =>
                                        feld("schulferien_von", e.target.value)
                                      }
                                      disabled={!canEdit}
                                    />
                                    <input
                                      ...
                                      onChange={(e) =>
                                        feld("schulferien_bis", e.target.value)
                                      }
                                      disabled={!canEdit}
                                    />
                                  </>
                            ...
                            {checkboxZeile(
                              ...
                              <>
                                <input
                                  ...
                                  onChange={(e) =>
                                    feld("rente_seit", e.target.value)
                                  }
                                  disabled={!canEdit}
                                />
                                <input
                                ...
                                <input
                                  ...
                                  onChange={(e) =>
                                    feld("rente_traeger", e.target.value)
                                  }
                                  disabled={!canEdit}
                                />
                              </>
                            ...
                            {checkboxZeile(
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  feld("hausmann_seit", e.target.value)
                                }
                                disabled={!canEdit}
                              />
                            )}
                          ...
                          <div className="rounded border border-neutral-200 p-3">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                feld("lebensunterhalt_sonstiges", e.target.value)
                              }
                              disabled={!canEdit}
                          ...
                          <div className="rounded border border-amber-300 bg-amber-50 p-3">
                            <p className="text-sm font-medium text-amber-900">
                              Bisherige Arbeitstage in Deutschland (dieses
                              Kalenderjahr, bei anderen Arbeitgebern) - zählt
                              für die 90-Tage-Grenze
                            </p>
                            <div className="mt-2 flex flex-wrap items-center gap-2">
                              <input
                                ...
                                placeholder="Tage"
                                value={
                                  entwurf.vorbeschaeftigung_deutschland_tage ?? ""
                                }
                                onChange={(e) =>
                                  feld(
                              ...
                              <input
                                ...
                                value={
                                  entwurf.vorbeschaeftigung_deutschland_arbeitgeber ??
                                  ""
                                }
                                onChange={(e) =>
                                  feld(
                              ...
                              <label className="flex items-center gap-1 text-xs">
                                <input
                                  type="checkbox"
                                  checked={
                                    entwurf.ausgeloest_durch_lohnprogramm_hinweis
                                  }
                                  onChange={(e) =>
                                    feld(
                          ...
                          <div className="rounded border border-neutral-200 p-3">
                            <div className="flex items-center justify-between">
                              <p className="text-sm font-medium">
                                8. Bisherige Beschäftigungen im Kalenderjahr
                                (In- oder Ausland, nur zur Dokumentation)
                              </p>
                              {canEdit && (
                                <button
                                  type="button"
                                      ...
                                      {canEdit && (
                                        <td>
                                          <button
                                            ...
                                            className="btn-secondary text-xs"
                                            onClick={() =>
                                              vorbeschaeftigungZeileEntfernen(idx)
                                            }
                                          >
                                            Entfernen
                                          </button>
                          ...
                          <label className="text-sm">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                feld("ausgefuellt_am", e.target.value)
                              }
                              disabled={!canEdit}
                            />
                          </label>
